...
void globle_esd_check_te_func()
{
 	atomic_set(&esd_ext_te_event, 1);
	if(primary_display_esd_check_task != NULL)
	{
	        wake_up_interruptible(&esd_ext_te_wq);
	}
}
EXPORT_SYMBOL(globle_esd_check_te_func);
...
int primary_display_init(char *lcm_name, unsigned int lcm_fps)
{
	...
	char more[64];
	sprintf(more, "%d * %d", lcm_param->width, lcm_param->height);
	hw_module_info_add_item("LCM", lcm_name, " ", 0, more);
