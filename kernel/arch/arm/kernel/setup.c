...
extern struct custom_virtual_date custom_date;
...
static int c_show(struct seq_file *m, void *v)
{
	...
	if (compat)
		seq_printf(m, "Processor\t: %s rev %d (%s)\n",
		   cpu_name, custom_date.cpu_id? custom_date.cpu_id : read_cpuid_id() & 15, elf_platform);
	...
	if (compat)
		c_show_features(m, cpuid);
	seq_printf(m, "Hardware\t: %s\n", custom_date.cpu_name[0]? custom_date.cpu_name : machine_name);
