...
# Fill MBR & EBR table -----------------------------------------------------
	for ($iter=0; $iter<@PARTITION_FIELD; $iter++) {   
		if($PARTITION_FIELD[$iter] eq "MBR"){
			...
			$mbr_size_block = ($SIZE_FIELD_KB[$iter])*2; 
		}
		if($PARTITION_FIELD[$iter] eq "EBR1"){
			$ebr1_start_block = ($START_FIELD_Byte[$iter]-$mbr_start)/512;
			$ebr1_size_block = ($SIZE_FIELD_KB[$iter])*2; 
		}
		if($PARTITION_FIELD[$iter] eq "EBR2"){
			$ebr2_start_block = ($START_FIELD_Byte[$iter]-$mbr_start)/512;
			$ebr2_size_block = ($SIZE_FIELD_KB[$iter])*2; 
		}
	...
	print "MBR content 1st(EBR1) start block: $BR[0][1][0][1] size block: $BR[0][1][0][2] \n";
	print "MBR content 2nd(cache) start block: $BR[0][1][1][1] size block: $BR[0][1][1][2] \n";
	print "MBR content 3rd(userdata) start block: $BR[0][1][2][1] size block: $BR[0][1][2][2] \n";
	print "MBR content 4th(fat) start block: $BR[0][1][3][1] size block: $BR[0][1][3][2] \n";
	#EBR1
	$BR[1][1][0][1] = $p5_start_block - $ebr1_start_block;
	$BR[1][1][0][2] = $p5_size_block;
	$BR[1][1][1][1] = $ebr2_start_block - $ebr1_start_block;
	$BR[1][1][1][2] = $p6_size_block + $ebr2_size_block;
	print "EBR1 content 1st(sec_ro) start block: $BR[1][1][0][1] size block: $BR[1][1][0][2] \n";
	print "EBR1 content 2nd(rest) start block: $BR[1][1][1][1] size block: $BR[1][1][1][2] \n";


	#EBR2
	$BR[2][1][0][1] = $p6_start_block-$ebr2_start_block;
	$BR[2][1][0][2] = $p6_size_block;
	print "EBR2 content 1st(system) start block: $BR[2][1][0][1] size block: $BR[2][1][0][2] \n";
