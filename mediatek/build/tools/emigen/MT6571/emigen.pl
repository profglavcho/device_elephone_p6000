...
sub check_EMI_setting
{
    ...
    for (1..$CUSTOM_MEM_DEV_OPTIONS_LOCAL->{COMBO_MEM_ENTRY_COUNT})
    {
        ...
        print "1.count:$CUSTOM_MEM_DEV_OPTIONS_LOCAL->{COMBO_MEM_ENTRY_COUNT},$_\n";
...
#            print "2.flash_id :$flash_id_cmp\n";
        # Limit 1: assert when more than 1 discrete LPDDR1
        ...
        # Limit 2: assert when more than 1 discrete PCDDR3
        ...
        # Limit 3: assert when the vendor ID or dram size of discrete LPDDR2/LPDDR3 are the same
        ...
        # Limit 4: assert when the flash ID or the vendor ID or dram size of MCP LPDDR2/LPDDR3 are the same
            if (   $mem_type eq "MCP(NAND+DDR2)" 
                || $mem_type eq "MCP(NAND+LPDDR3)"
                || $mem_type eq "MCP(eMMC+DDR2)"
                || $mem_type eq "MCP(eMMC+LPDDR3)")
            {
               if (   ($mem_type_cmp =~ /DDR2|LPDDR3/) 
                   && ($dram_vendor_id eq $dram_vendor_id_cmp) 
                   && ($emi_size eq $emi_size_cmp) 
        ...
        # Limit 5: assert when 2 or more LPDDR1 which the flash ID are the same
            if ($mem_type =~ /DDR1/ && $mem_type_cmp  =~ /DDR1/ ) 
            {
               if ($flash_id eq $flash_id_cmp)
               {
                   die "[Error] Combo MCP DRAM(LPDDR1, PCDDR3) cannot support two part number which the flash ID are the same\n" ;
               }
            }
        # Limit 6: assert when 2 or more PCDDR3 which the flash ID are the same
            if ($mem_type =~ /PCDDR3/ && $mem_type_cmp  =~ /PCDDR3/ ) 
            {
               if ($flash_id eq $flash_id_cmp)
               {
                   die "[Error] Combo MCP DRAM(LPDDR1, PCDDR3) cannot support two part number which the flash ID are the same\n" ;
        ...
        # Limit 7: assert when Discrete and MCP which the flash ID are the same
            if (    ($mem_type =~ /Discrete/ && $mem_type_cmp  =~ /MCP/) 
                 || ($mem_type =~ /MCP/ && $mem_type_cmp  =~ /Discrete/) ) 
            {
                if ($flash_id eq $flash_id_cmp)
                {
                    die "[Error] Combo MCP DRAM (Discrete and MCP) cannot support two part number which the flash ID are the same\n" ;
                }
            }
        }
    }
        	   ...
        	   for(0..$#$mareas)
        	   {
        	   	  ...
        	   	  elsif($col >= 0 && $col <=8)
        	   	  {
        	   	  	...
        	   	  	$value = $sheet->get_cell(0,$col)->value();
                            #print "row :$row,col : $col, $mareas->[$_]->[1], $mareas->[$_]->[3], $value\n";
        	   	  	return $value;
        	   	  }
        	      elsif($col >= $mareas->[$_]->[1] && $col <= $mareas->[$_]->[3])
        	      {
