...
sub DeviceListParser_LPSDRAM
{
    ...
    while ($iter<$num_part_found)
    {
        ...
        $DRAMC_MCKDLY_VAL[$id]                  = &xls_cell_value($Sheet, $_, $platform_scan_idx++) ;
...
#3 DDR3    
        ...
        else
        {
            if ($hex_val & 0x20000) 
            {
                print "[Warning] CONA value with single rank:$DENSITY[$id],$EMI_CONA_VAL[$id]";
            }
        }
...
=cut
        ...
        $EMI_SETTINGS[$id] = $EMI_SETTINGS[$id] . $DRAMC_MCKDLY_VAL[$id] . ",\t\t/* DRAMC_MCKDLY_VAL */\n\t\t" ;
...
sub write_tag_one_element()
{
    ...
    print FILE pack ("L", hex (lc($DRAMC_MCKDLY_VAL[$id]))) ;        # DRAMC_MCKDLY_VAL
    $fs = $fs + 4 ;
