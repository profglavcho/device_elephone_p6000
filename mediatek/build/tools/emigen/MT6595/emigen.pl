...
#=cut
    ...
    unless($ENV{PROJECT} eq "mt6595_evb"){
...
# LPDDR3    
	...
	#rank size check END
        ...
        elsif ($DEV_TYPE[$id] eq "MCP(eMMC+LPDDR2)")
        {
            ...
            $DEV_TYPE2[$id] = "02" ;
	    $MCP_LPDDR2 = $MCP_LPDDR2 + 1 ;
	    die "[Error][EMIgen] no support eMMC+LPDDR2.";  
        }
        elsif ($DEV_TYPE[$id] eq "MCP(eMMC+LPDDR3)")
        {
            ...
            $DEV_TYPE2[$id] = "03" ;
	    $MCP_LPDDR3 = $MCP_LPDDR3 + 1 ;
	    die "[Error][EMIgen] no support eMMC+LPDDR3.";
        }
	...
	$nand_emmc_id_len = $ID_Length[$id];
        ...
        $EMI_SETTINGS[$id] = $EMI_SETTINGS[$id] . $DRAMC_RKCFG_VAL[$id] . ",\t\t/* DRAM_CRKCFG_VAL*/\n\t\t" ;
...
sub write_tag()
{
    ...
    print FILE pack("a24", "MTK_BLOADER_INFO_v15");
...
sub write_tag_one_element_default_LPDDR3()
{
    ...
    #print FILE pack ("L", hex (lc("0x20102017"))) ;         # EMI_CONA_VAL
    print FILE pack ("L", hex (lc("0x50535057"))) ;         # EMI_CONA_VAL
    $fs = $fs + 4 ;
    ...
    print FILE pack ("L", hex (lc("0x77FD474B"))) ;      # DRAMC_ACTIM_VAL
    $fs = $fs + 4 ;
    
    print FILE pack ("L", hex (lc("0x11000000"))) ;  # DRAMC_GDDR3CTL1_VAL
    $fs = $fs + 4 ;
    
    print FILE pack ("L", hex (lc("0x00048403"))) ;      # DRAMC_CONF1_VAL
    $fs = $fs + 4 ;

    print FILE pack ("L", hex (lc("0x000053B1"))) ;    # DRAMC_DDR2CTL_VAL
    $fs = $fs + 4 ;

    print FILE pack ("L", hex (lc("0xBFC40401"))) ;    # DRAMC_TEST2_3_VAL
    $fs = $fs + 4 ;
    
    print FILE pack ("L", hex (lc("0x0000006C"))) ;      # DRAMC_CONF2_VAL
    $fs = $fs + 4 ;
    
    print FILE pack ("L", hex (lc("0xD1646142"))) ;    # DRAMC_PD_CTRL_VAL
    $fs = $fs + 4 ;
    ...
    print FILE pack ("L", hex (lc("0x002145C1"))) ;        # DRAM_CRKCFG_VAL
    $fs = $fs + 4 ;
    ...
    {
        #ddr2
        print FILE pack ("L", hex (lc("0x00830001"))) ;        # DDR2_MODE_REG1
        $fs = $fs + 4 ;

        print FILE pack ("L", hex (lc("0x001A0002"))) ;        # DDR2_MODE_REG2
        $fs = $fs + 4 ;
...
sub write_tag_one_element()
{
    ...
    print FILE pack ("L", hex (lc($DRAM_CRKCFG_VAL[$id]))) ;        # DRAM_CRKCFG_VAL
    $fs = $fs + 4 ;
