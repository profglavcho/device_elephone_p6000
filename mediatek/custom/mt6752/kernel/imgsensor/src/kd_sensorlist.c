		...
		for ( i = KDIMGSENSOR_INVOKE_DRIVER_0 ; i < KDIMGSENSOR_MAX_INVOKE_DRIVERS ; i++ ) {
			if(DUAL_CAMERA_NONE_SENSOR != g_invokeSocketIdx[i]){
		        ...
		        if (sensorID == 0) {    //not implement this feature ID
		            ...
		            printk(" Sensor found ID = 0x%x g_invokeSocketIdx[i]=%d\n", sensorID , g_invokeSocketIdx[i]);
				if(first_enter){
					hw_module_info_add_item("Main Camera", g_invokeSensorNameStr[i], " ", sensorID, " ");
					first_enter = 0;
				}
				else{
					hw_module_info_add_item("Sub Camera", g_invokeSensorNameStr[i], " ", sensorID, " ");
				}
		            err = ERROR_NONE;
		        }
		        if(ERROR_NONE != err)
