...
static struct mir3da_i2c_data *obj_i2c_data = NULL;
...
{    
    ...
    else
    {
      ...
      if(bxstk)
			   acc[MIR3DA_AXIS_X] =squareRoot(1024*1024 - acc[MIR3DA_AXIS_Y]*acc[MIR3DA_AXIS_Y] - acc[MIR3DA_AXIS_Z]*acc[MIR3DA_AXIS_Z]);
      if(bystk)
			   acc[MIR3DA_AXIS_Y] =squareRoot(1024*1024 - acc[MIR3DA_AXIS_X]*acc[MIR3DA_AXIS_X] - acc[MIR3DA_AXIS_Z]*acc[MIR3DA_AXIS_Z]);
      if(bzstk)
...
static int gsensor_enable_nodata(int en)
{
	...
	{
		enable_status = !sensor_power;
		err = mir3da_setPowerMode( obj_i2c_data->client, !sensor_power);
...
static int gsensor_set_delay(u64 ns)
{
    ...
    if (err)
    {
	 ...
	 err = mir3da_set_odr(obj_i2c_data->client,value); 
	 if(err) {
		 MI_ERR("mir3da_set_odr failed !");
		 err = -EINVAL;
	 }	
...
static int gsensor_get_data(int* x ,int* y,int* z, int* status)
{
    ...
    mutex_lock(&bma222_op_mutex);
	mir3da_readSensorData(obj_i2c_data->client, buff);
	mutex_unlock(&bma222_op_mutex);
...
static int mir3da_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	hw_module_info_add_item("G-sensor", MIR3DA_DRV_NAME, " ", 0x26, " ");
