...
//    , m_eAETargetMode(AE_MODE_MVHDR_TARGET)
...
MRESULT AeMgr::PreviewAEInit(MINT32 i4SensorIdx, Param_T &rParam)
{
    ...
    AAASensorMgr::getInstance().getSensorSyncinfo(m_eSensorDev, &i4SutterDelay, &i4SensorGainDelay, &i4IspGainDelay);
...
MRESULT AeMgr::setAELock(MBOOL bAELock)
{
    if (m_bAELock != bAELock) {
        MY_LOG("[AeMgr::setAELock] m_bAELock: %d %d AEState:%d %d\n", m_bAELock, bAELock, m_eAEState, m_ePreAEState);
        if (bAELock) { // AE lock
            m_bAELock = MTRUE;
...
{
    * i4ActiveCycle = m_i4IspGainDelayFrames + 1;
