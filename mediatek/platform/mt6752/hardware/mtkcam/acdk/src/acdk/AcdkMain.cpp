...
//
    ...
    if(1)
    {
	 MINT32 u32shtter_time_ms;
 	MINT32	u32gain;

	   if(mSensorDev == SENSOR_DEV_MAIN )
	   {
		u32shtter_time_ms = 2000;
		u32gain = 0x8000;
	   }
	   else{
		u32shtter_time_ms = 64000;
		u32gain = 8192;
	        err = m_pSensorHalObj->sendCommand(mSensorDev,
                ...
                MINT32  const isSaveImg)
{
    ...
    err = m_pAcdkMhalObj->acdkMhalCaptureStart(&mAcdkMhalCapParam);   // do AF
    if(err != ACDK_RETURN_NO_ERROR)
    {
        ACDK_LOGE("m_pAcdkMhalObj->acdkMhalCaptureStart(err=0x%x)",err);
        return ACDK_RETURN_API_FAIL;
...
)
{
    ...
    else if(a_u4Ioctl == ACDK_CMD_CAPTURE)
    {
        ...
        switch(pCapConfig->eOutputFormat)
        {
            ...
            if(ACDK_OPT_FACTORY_MODE == mOperaMode)
            {
