...
// ************************************************************************
#define RETRY_LOAD_COUNT		3	//retry load image count
...
//	unsigned long begin;
	...
	do
	{
	    /* check a hash of the contents in the header so boot images can be
		...
		SHA_init(&ctx);
		SHA_update(&ctx, g_kmem_off - MKIMG_HEADER_SZ, g_boot_hdr->kernel_size);
		SHA_update(&ctx, &g_boot_hdr->kernel_size, sizeof(g_boot_hdr->kernel_size));
		SHA_update(&ctx, g_rmem_off - MKIMG_HEADER_SZ, g_boot_hdr->ramdisk_size);
		SHA_update(&ctx, &g_boot_hdr->ramdisk_size, sizeof(g_boot_hdr->ramdisk_size));
		SHA_update(&ctx, second_data, g_boot_hdr->second_size);
		SHA_update(&ctx, &g_boot_hdr->second_size, sizeof(g_boot_hdr->second_size));
		sha = SHA_final(&ctx);

		if (!memcmp(g_boot_hdr->id, sha,
			SHA_DIGEST_SIZE > sizeof(g_boot_hdr->id) ? sizeof(g_boot_hdr->id) : SHA_DIGEST_SIZE)) {
			printf("SHA is ok\n");
			break;
		}else {
			printf("SHA is error(%d) !!\n",retry);
#ifdef MTK_EMMC_SUPPORT
#ifdef MTK_NEW_COMBO_EMMC_SUPPORT
			len = dev->read(dev, start_addr, (uchar*)addr, g_bimg_sz/2, part->part_id);
			len = dev->read(dev, start_addr + g_bimg_sz/2, (uchar*)addr + g_bimg_sz/2, g_bimg_sz/2, part->part_id);
#else
			len = dev->read(dev, start_addr, (uchar*)addr, g_bimg_sz/2);
			len = dev->read(dev, start_addr + g_bimg_sz/2, (uchar*)addr + g_bimg_sz/2, g_bimg_sz/2);
#endif
#else
			len = dev->read(dev, start_addr, (uchar*)addr, g_bimg_sz/2);
			len = dev->read(dev, start_addr + g_bimg_sz/2, (uchar*)addr + g_bimg_sz/2, g_bimg_sz/2);
...
//	unsigned long begin;
	...
	do
	{
		...
		if (!memcmp(g_boot_hdr->id, sha,
			SHA_DIGEST_SIZE > sizeof(g_boot_hdr->id) ? sizeof(g_boot_hdr->id) : SHA_DIGEST_SIZE)) {
			printf("recovery SHA is ok\n");
			break;
		}else {
			printf("recovery SHA is error(%d) !!\n",retry);
#ifdef MTK_EMMC_SUPPORT
#ifdef MTK_NEW_COMBO_EMMC_SUPPORT
			len = dev->read(dev, start_addr, (uchar*)addr, g_rcimg_sz/2, part->part_id);
			len = dev->read(dev, start_addr + g_rcimg_sz/2, (uchar*)addr + g_rcimg_sz/2, g_rcimg_sz/2, part->part_id);
#else
			len = dev->read(dev, start_addr, (uchar*)addr, g_rcimg_sz/2);
			len = dev->read(dev, start_addr + g_rcimg_sz/2, (uchar*)addr + g_rcimg_sz/2, g_rcimg_sz/2);
#endif
#else
			len = dev->read(dev, start_addr, (uchar*)addr, g_rcimg_sz/2);
			len = dev->read(dev, start_addr + g_rcimg_sz/2, (uchar*)addr + g_rcimg_sz/2, g_rcimg_sz/2);
