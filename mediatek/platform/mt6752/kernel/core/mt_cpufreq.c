...
static void _mt_cpufreq_set(enum mt_cpu_dvfs_id id, int new_opp_idx)
{
    ...
    // this will be changed possibly
    freqs.new = mt_cpufreq_get_freq_by_idx(id, new_opp_idx);
...
#ifdef CONFIG_CPU_FREQ
    if (policy) {
        // update freq by new_opp_idx (without turbo)
        freqs.new = mt_cpufreq_get_freq_by_idx(id, new_opp_idx);
        for_each_online_cpu(cpu) {
            freqs.cpu = cpu;
...
void mt_cpufreq_enable_by_ptpod(enum mt_cpu_dvfs_id id)
{
     ...
     *  3. PTPOD is enabled
     */    
    if (mt6325_chip_ver >= PMIC6325_E3_CID_CODE && p->cpu_level == CPU_LEVEL_1) {
